!function (e) {
    ...
    var t = {
        ...
        tabs: function () {
            // Usando jQuery para delegar el evento a nivel del documento:
            $(document).on("click", "#tabs-container a", function (event) {
                event.preventDefault();

                var $a = $(this);
                var href = $a.attr("href"); // e.g., "#tab1"

                // Actualizamos la clase active en los <li>
                $("#tabs-container li").removeClass("active");
                $a.closest("li").addClass("active");

                // Asumimos que tienes una sección con la clase .tab_content y cada tab es un hijo identificado por su ID (tab1, tab2, etc)
                $(".tab_content").children().removeClass("active");
                $(href).addClass("active");

                // Opcional: scroll suave a la sección activa
                $("html, body").animate({
